/* MinWinLib.c */
#include <stdbool.h>
// Get the files for this at https://glad.dav1d.de/#language=c&specification=egl&api=egl%3D1.5&loader=on
// Then use `#define MWL_USE_GLAD "path/to/glad_egl.h"` to actually include it
#ifdef MWL_USE_GLAD
#include MWL_USE_GLAD
#endif // MWL_USE_GLAD
...
#define MWL_GLAD_NOT_INCLUDED -10
...
const int MWL_RESIZABLE_WINDOW = 0x01;
 ...
 * @returns `MWL_SUCCESS`
 *
 * `MWL_createWindow(&myWindow, "Hello, world!", 600, 400, MWL_RESIZABLE_WINDOW)`
 */
int MWL_createWindow(MWL_Window *window, const char *t, int w, int h, int flags)
{
    ...
    // Do this to stop printf blockages?
...
#elif __linux__
    // TODO figure out how to show the cursor when it is moved outside the window
    if (window->cursorVisible)
    {
        XFixesShowCursor(mwl_internalWindow.display, mwl_internalWindow.window);
    }
    else
    {
        XFixesHideCursor(mwl_internalWindow.display, mwl_internalWindow.window);
    }
#endif
}
